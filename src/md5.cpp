...
static unsigned char PADDING[64] = { 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
string digest(const string& s)
{
    MD5_CTX mdContext;
    MD5Init(&mdContext);
    MD5Update(&mdContext, (unsigned char *) s.c_str(), s.length());
    MD5Final(&mdContext);
    std::stringstream ss;
    char buf[8];
    for (int i = 0; i < 16; i++)
        ss << (sprintf(buf, "%02x", mdContext.digest[i]), buf);
    return ss.str();
}
